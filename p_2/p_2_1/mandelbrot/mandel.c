...
int main(int argc, char *argv[]) {
  mandelPars pars;
  int i,k,x,y,level,*rdy,*res,*jobTaken,iret,*pid;
  int xoff,yoff;
  ...
  pid = (int *) malloc(sizeof(int)*pars.slices);
  worker_mtx = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t)*pars.slices);
  ...
  for ( i=0; i<pars.slices; i++ ) {
    if ( pthread_mutex_init(&worker_mtx[i], NULL) != 0 ) {
  ...
  if ( pthread_mutex_init(&job_mtx, NULL) != 0 ) {
    printf("\n job_mutex init failed\n");
    return 1;
  }
  // "Born" the threads
  i=0;
  while(i<pars.slices){  
    iret = pthread_create ( &thread, NULL, workerJob, (void *)&pid[i]);
  ...
  while (1) {
    ...
    i=0;
    while ( i < pars.slices ){
      pthread_mutex_unlock(&worker_mtx[i]);
      i++;  
    }
