...
int main(int argc, char *argv[]) {
  mandelPars pars;
  int i,k,x,y,level,*rdy,*res,*jobTaken,iret;
  int xoff,yoff;
  long double reStep,imStep;
  thread_arg *thread_pars,*curr,*tmp;
  ...
  jobTaken = (int *) malloc(sizeof(int)*pars.slices);
  /* open window for drawing results */

  openDisplay();
  openWin(argv[0], WinW, WinH);

  // Create a list to give as a parameter to threads
  thread_pars = ( thread_arg*)malloc (sizeof(thread_arg));
  for ( i=0; i<NumOfThreads; i++ ) {

    curr = ( thread_arg*)malloc (sizeof(thread_arg));
    curr->pars = &pars;
    curr->num_of_threads = NumOfThreads;
    curr->id_num = i;
    curr->nxt = thread_pars;
    thread_pars = curr;
  }

  // "Born" the threads
  for ( i=0,curr = thread_pars; i<NumOfThreads; i++,curr = curr->nxt){
    iret = pthread_create ( &thread[i], NULL, workerJob, (void *)curr);
  ...
  while (1) {
    ...
    for (i=0; i<pars.slices; i++) { 
      rdy[i]=0; /* reset ready state */
      jobTaken[i] = 0; /*reset jobTaken state */
    }

    calcMandel(&pars,res,rdy,jobTaken);
  ...
  free(rdy);
  free(jobTaken);
  for ( i=0,curr = thread_pars;i<NumOfThreads; i++, curr=tmp) {
    tmp = curr->nxt;
    free(curr);
  }
